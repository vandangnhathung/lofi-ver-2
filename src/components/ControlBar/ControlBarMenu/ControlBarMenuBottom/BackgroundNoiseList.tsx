...
import {BackgroundSound, setNewSounds,} from "@/redux/reducers/backgroundSoundSlice";
      ...
      <ul className={`flex flex-col gap-2 h-full`}>
        ...
        {limitedBackgroundSounds.map((sound, index) => (
            <BackgroundNoiseItem
                ...
                soundName={sound.name}
                index={index}
                className={`${mixMore ? "opacity-100 transition-all" : "duration-0 opacity-0 cursor-default pointer-events-none"}`}
                volume={sound.volume ?? 0} // Provide a default value of 0 if sound.volume is undefined
            />
        ))}
      </ul>
